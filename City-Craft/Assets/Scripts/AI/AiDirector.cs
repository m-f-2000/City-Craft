...
namespace City.AI
{
    public class AiDirector : MonoBehaviour
    {
        ...
        {
            foreach (var house in placement.GetAllHouses())
            {
                TrySpawningAnAgent(house, placement.GetRandomSpecialStrucutre());
            }
            foreach (var specialStructure in placement.GetAllSpecialStructures())
            {
                TrySpawningAnAgent(specialStructure, placement.GetRandomHouseStructure());
            }
        }
        ...
        private void TrySpawningAnAgent(StructureModel startStructure, StructureModel endStructure)
        {
            if(startStructure != null && endStructure != null)
            {
                ...
                var startMarkerPosition = placement.GetStructureAt(startPosition).GetNearestMarkerTo(startStructure.transform.position);
                var endMarkerPosition = placement.GetStructureAt(endPosition).GetNearestMarkerTo(endStructure.transform.position);

                var agent = Instantiate(GetRandomPedestrian(), startMarkerPosition, Quaternion.identity);
                var path = placement.GetPathBetween(startPosition, endPosition, true);
                if(path.Count > 0)
                {
                    path.Reverse();
        ...
        private void CreatAGraph(List<Vector3Int> path)
        {
            ...
            for (int i = 0; i < path.Count; i++)
            {
                ...
                var roadStructure = placement.GetStructureAt(currentPosition);
                ...
                foreach (var marker in markersList)
                {
                    ...
                    {
                        var nextRoadStructure = placement.GetStructureAt(path[i + 1]);
                        if (limitDistance)
                        {
